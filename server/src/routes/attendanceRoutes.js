...
router.get("/report", requirePermission("attendance.manage"), async (req, res, next) => {
  try {
    ...
    if (from > to) return res.status(400).json({ message: "The from date must be before the to date" });
    if (daysBetween(from, to) > REPORT_LIMIT_DAYS) return res.status(400).json({ message: "Reports can cover a maximum of 366 days" });

    const query = { attendanceDate: { $gte: from, $lte: to } };
    if (req.query.userId) {
      if (!mongoose.isValidObjectId(req.query.userId)) return res.status(400).json({ message: "Invalid employee filter" });
    ...
    res.json({
      ...
      overview: {
        ...
        notMarkedToday: Math.max(0, activeEmployees.length - markedToday.size),
