...
import { portalApi, setWorkspaceSlug } from "../api/client.js";
...
export default function PortalAccess({ mode = "login" }) {
  ...
  async function submit(event) {
    ...
    try {
      if (mode === "activate") { if (values.password !== values.confirmPassword) throw new Error("Passwords do not match"); const result = await portalApi(`/portal/invitation/${encodeURIComponent(token)}/activate`, { method: "POST", body: { password: values.password } }); setStatus({ message: result.message }); }
