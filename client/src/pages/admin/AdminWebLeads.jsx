...
import { Bell, Building2, CalendarClock, ClipboardList, FileCheck2, Filter, LayoutDashboard, Mail, MessageSquareText, Plus, Search, Send, Settings, Target, UserCheck, UsersRound } from "lucide-react";
...
function DashboardView({ data, manager }) {
  ...
  return <><div className="webcrm-kpi-grid">{cards.map(([label, value]) => <article key={label}><span>{label}</span><strong>{Number(value || 0).toLocaleString()}</strong></article>)}</div>{manager && <section className="card webcrm-agent-summary"><div className="webcrm-section-title"><UsersRound size={20} /><div><h2>Prospects by agent</h2><p>Current filtered performance snapshot for team coaching.</p></div></div><div className="webcrm-agent-summary-grid">{(data.byAgent || []).map((row) => <article key={row._id?.user}><strong>{row._id?.name || "Unknown agent"}</strong><span>{row.prospects} prospects</span><small>{row.interested} interested · {row.qualified} qualified · {row.accepted} accepted</small></article>)}{!data.byAgent?.length && <p className="muted">No agent activity in this period.</p>}</div></section>}<section className="card webcrm-recent"><div className="webcrm-section-title"><ClipboardList size={20} /><div><h2>Recent prospect activity</h2><p>Most recently updated actionable records.</p></div></div><ProspectTable items={data.recent || []} compact /></section></>;
}
...
function ProspectDetail({ prospect, meta, user, onRefresh }) {
  ...
  return <section className={`webcrm-detail${busy ? " is-busy" : ""}`} aria-busy={busy}>
...
export default function AdminWebLeads({ mode = "dashboard" }) {
  ...
  const [filters, setFilters] = useState({ search: "", status: "", category: "", service: "", agent: "", from: "", to: "" });
  ...
  return <>
    ...
    {!loading && listMode && <>
      <section className="card webcrm-filter-bar"><div className="webcrm-section-title"><Filter size={19} /><div><h2>Search and filter</h2><p>{meta.manager ? "Manager view includes all external agents." : "Only prospects submitted by your account are searchable."}</p></div></div><form onSubmit={(e) => { e.preventDefault(); mode === "followups" ? loadFollowUps(1) : loadList(1); }}><div className="input-with-icon"><Search size={17} /><input placeholder="Business, contact, phone, email, website or postcode" value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} /></div><select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><option value="">All statuses</option>{meta.statuses.map((item) => <option key={item}>{item}</option>)}</select><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">All categories</option>{meta.categories.filter((item) => item.isActive !== false).map((item) => <option key={item._id || item.name}>{item.name}</option>)}</select><select value={filters.service} onChange={(e) => setFilters({ ...filters, service: e.target.value })}><option value="">All services</option>{meta.services.map((item) => <option key={item}>{item}</option>)}</select>{meta.manager && <select value={filters.agent} onChange={(e) => setFilters({ ...filters, agent: e.target.value })}><option value="">All agents</option>{meta.agents.map((item) => <option key={item._id} value={item._id}>{item.name}</option>)}</select>}<input aria-label="Created from" type="date" value={filters.from} onChange={(e) => setFilters({ ...filters, from: e.target.value })} /><input aria-label="Created to" type="date" value={filters.to} onChange={(e) => setFilters({ ...filters, to: e.target.value })} /><button className="button">Apply filters</button></form></section>
