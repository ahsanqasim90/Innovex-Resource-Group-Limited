...
export default function AdminWebLeads({ mode = "dashboard" }) {
  ...
  return <>
    ...
    <StatusMessage status={status} />
    {!loading && notifications.some((item) => !item.read) && <section className="webcrm-notifications">{notifications.filter((item) => !item.read).slice(0, 4).map((item) => <article key={item._id}><Bell size={17} /><div><strong>{item.title}</strong><p className="testimonial-review-copy">{item.message}</p></div><button onClick={async () => { await api(`/web-leads/notifications/${item._id}/read`, { method: "PATCH" }); setNotifications((current) => current.map((notice) => notice._id === item._id ? { ...notice, read: true } : notice)); }}>Mark read</button></article>)}</section>}
