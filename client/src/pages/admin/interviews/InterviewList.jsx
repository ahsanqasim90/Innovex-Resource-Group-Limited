...
export default function InterviewList({ interviews, onEdit, onDelete, onSelect, selectedId }) {
  return (
    <div className="table-wrap interview-table">
      <table>
        <thead><tr><th>Candidate</th><th>Job / Client</th><th>Interview</th><th>Status</th><th>Outcome</th><th>Revenue</th><th>Actions</th></tr></thead>
        <tbody>
          {interviews.map((item) => (
            <tr key={item._id} className={selectedId === item._id ? "selected-row" : ""}>
              ...
              <td><strong>£{Number(item.revenue || 0).toLocaleString()}</strong></td>
