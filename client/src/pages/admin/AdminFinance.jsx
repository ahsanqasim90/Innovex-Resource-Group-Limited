...
import { AlertTriangle, BadgePoundSterling, CalendarClock, CheckCircle2, Download, FileSpreadsheet, FileText, Landmark, Mail, Plus, ReceiptText, RefreshCw, Send, TrendingUp, Upload, XCircle } from "lucide-react";
...
const dateTimeLabel = (value) => value ? new Date(value).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short" }) : "-";
...
function InvoiceWorkspace({ invoices, senders, reload, status, setStatus }) {
  const [form, setForm] = useState(emptyInvoice); const [editing, setEditing] = useState(null); const [selected, setSelected] = useState(null); const [saving, setSaving] = useState(false); const [sending, setSending] = useState(false); const [message, setMessage] = useState(""); const [cc, setCc] = useState(""); const [scheduledAt, setScheduledAt] = useState("");
  ...
  const openInvoice = (invoice) => { setSelected(invoice); setCc((invoice.cc || []).join(", ")); setScheduledAt(localDateTimeValue(invoice.scheduledSendAt)); setMessage(invoice.scheduledMessage || ""); };
  const send = async (reminder = false) => { if (!selected) return; setSending(true); try { const result = await api(`/finance/invoices/${selected._id}/${reminder ? "remind" : "send"}`, { method: "POST", body: { fromEmail: selected.senderEmail || senders[0]?.address, message, cc } }); openInvoice(result.invoice); setStatus({ message: result.message }); setMessage(""); reload(); } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setSending(false); } };
  const schedule = async () => { if (!selected || !scheduledAt) return setStatus({ type: "error", message: "Choose a future delivery date and time." }); setSending(true); try { const result = await api(`/finance/invoices/${selected._id}/schedule`, { method: "POST", body: { fromEmail: selected.senderEmail || senders[0]?.address, message, cc, scheduledSendAt: new Date(scheduledAt).toISOString() } }); openInvoice(result.invoice); setStatus({ message: result.message }); reload(); } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setSending(false); } };
  const cancelSchedule = async () => { if (!selected) return; setSending(true); try { const result = await api(`/finance/invoices/${selected._id}/schedule`, { method: "DELETE" }); openInvoice(result.invoice); setStatus({ message: result.message }); reload(); } catch (error) { setStatus({ type: "error", message: error.message }); } finally { setSending(false); } };
  ...
  return <div className="finance-workspace"><InvoiceForm form={form} setForm={setForm} editing={editing} saving={saving} onSave={save} onCancel={() => { setEditing(null); setForm({ ...emptyInvoice, lineItems: [{ ...emptyLine }] }); }} />
    <aside className="finance-panel invoice-action-panel">{selected ? <>
      <span className="eyebrow">Invoice delivery</span><h2>{selected.invoiceNumber}</h2><p>{selected.clientName}</p>
      <div className="invoice-action-total"><span>Outstanding balance</span><strong>{money(selected.balanceDue)}</strong><small>Due {dateLabel(selected.dueDate)}</small></div>
      {selected.scheduledStatus === "Scheduled" && <div className="invoice-schedule-banner"><CalendarClock size={18} /><div><strong>Scheduled</strong><span>{dateTimeLabel(selected.scheduledSendAt)}</span></div></div>}
      {selected.scheduledStatus === "Failed" && <div className="invoice-schedule-banner failed"><AlertTriangle size={18} /><div><strong>Scheduled delivery failed</strong><span>{selected.scheduleError}</span></div></div>}
      <label><span>Send from</span><select value={selected.senderEmail || senders[0]?.address || ""} onChange={(e) => setSelected({ ...selected, senderEmail: e.target.value })}>{senders.map((sender) => <option value={sender.address} key={sender.address}>{sender.label} · {sender.address}</option>)}</select></label>
      <label><span>CC recipients</span><input value={cc} onChange={(e) => setCc(e.target.value)} placeholder="accounts@example.com, manager@example.com" /><small>Separate multiple addresses with commas.</small></label>
      <label><span>Optional email message</span><textarea rows="4" value={message} onChange={(e) => setMessage(e.target.value)} placeholder="A professional default message is already prepared." /></label>
      <label><span>Schedule delivery</span><input type="datetime-local" value={scheduledAt} min={localDateTimeValue(new Date())} onChange={(e) => setScheduledAt(e.target.value)} /></label>
      <div className="invoice-action-buttons"><button className="button" onClick={() => send(false)} disabled={sending}><Send size={16} /> Send now</button><button className="button secondary" onClick={schedule} disabled={sending || !scheduledAt}><CalendarClock size={16} /> Schedule invoice</button>{["Scheduled", "Failed"].includes(selected.scheduledStatus) && <button className="button danger" onClick={cancelSchedule} disabled={sending}><XCircle size={16} /> Cancel schedule</button>}<button className="button secondary" onClick={() => downloadFile(`/finance/invoices/${selected._id}/pdf`, `Innovex-Invoice-${selected.invoiceNumber}.pdf`)}><Download size={16} /> Download PDF</button>{selected.status !== "Draft" && selected.balanceDue > 0 && <button className="button secondary" onClick={() => send(true)} disabled={sending}><Mail size={16} /> Send reminder</button>}</div>
      <small className="invoice-sent-note">Successful emails are recorded in CRM history and copied to the selected mailbox Sent folder.</small>
    </> : <div className="finance-empty"><FileText /><strong>Select an invoice</strong><span>Choose a record below to download, send, schedule, remind or record payment.</span></div>}</aside>
    <section className="finance-panel finance-register"><div className="finance-panel-head"><div><span className="eyebrow">Invoice register</span><h2>Invoices and payment status</h2></div><span className="finance-count">{invoices.length} records</span></div><div className="table-wrap"><table><thead><tr><th>Invoice</th><th>Client</th><th>Issued / due</th><th>Status</th><th>Delivery</th><th>Total</th><th>Balance</th><th>Actions</th></tr></thead><tbody>{invoices.map((invoice) => <tr key={invoice._id} className={selected?._id === invoice._id ? "selected-row" : ""}><td><strong>{invoice.invoiceNumber}</strong><br /><span className="muted">{invoice.invoiceType}</span></td><td><strong>{invoice.clientName}</strong><br /><span className="muted">{invoice.billingEmail}</span></td><td>{dateLabel(invoice.issueDate)}<br /><span className="muted">Due {dateLabel(invoice.dueDate)}</span></td><td><span className={statusClass(invoice.status)}>{invoice.status}</span></td><td>{invoice.scheduledStatus === "Scheduled" ? <><span className="finance-status scheduled">Scheduled</span><br /><span className="muted">{dateTimeLabel(invoice.scheduledSendAt)}</span></> : invoice.scheduledStatus === "Failed" ? <span className="finance-status cancelled">Failed</span> : <span className="muted">{invoice.sentAt ? `Sent ${dateLabel(invoice.sentAt)}` : "Not sent"}</span>}</td><td>{money(invoice.total)}</td><td><strong>{money(invoice.balanceDue)}</strong></td><td><div className="compact-actions"><button className="button secondary small" onClick={() => openInvoice(invoice)}>Open</button><button className="button secondary small" onClick={() => edit(invoice)}>Edit</button>{invoice.status === "Draft" && <button className="button danger small" onClick={() => remove(invoice)}>Delete</button>}</div></td></tr>)}</tbody></table></div>{!invoices.length && <div className="finance-empty"><FileText /><strong>No invoices yet</strong><span>Create your first draft invoice above.</span></div>}</section>
  </div>;
}
